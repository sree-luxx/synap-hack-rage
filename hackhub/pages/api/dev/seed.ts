...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    let teamAId = "demo-team-a";
    let teamBId = "demo-team-b";
    let prismaOk = true;
    ...
    try {
      ...
      const teamA = await prisma.team.upsert({
        where: { id: "demo-team-a" },
        update: {},
        create: { id: "demo-team-a", name: "Team Alpha", eventId: event.id },
      });
      const teamB = await prisma.team.upsert({
        where: { id: "demo-team-b" },
        update: {},
        create: { id: "demo-team-b", name: "Team Beta", eventId: event.id },
      });
      ...
      await prisma.teamMember.upsert({
        where: { teamId_userId: { teamId: teamA.id, userId: p1.id } },
        update: {},
        create: { teamId: teamA.id, userId: p1.id, role: "leader" } as any,
      });
      await prisma.teamMember.upsert({
        where: { teamId_userId: { teamId: teamA.id, userId: p2.id } },
        update: {},
        create: { teamId: teamA.id, userId: p2.id } as any,
      });
      await prisma.teamMember.upsert({
        where: { teamId_userId: { teamId: teamB.id, userId: p3.id } },
        update: {},
        create: { teamId: teamB.id, userId: p3.id, role: "leader" } as any,
      });

      await prisma.registration.upsert({
        where: { userId_eventId: { userId: p1.id, eventId: event.id } as any },
        update: {},
        create: { userId: p1.id, eventId: event.id, teamId: teamA.id },
      });
      await prisma.registration.upsert({
        where: { userId_eventId: { userId: p2.id, eventId: event.id } as any },
        update: {},
        create: { userId: p2.id, eventId: event.id, teamId: teamA.id },
      });
      await prisma.registration.upsert({
        where: { userId_eventId: { userId: p3.id, eventId: event.id } as any },
        update: {},
        create: { userId: p3.id, eventId: event.id, teamId: teamB.id },
      });
    } catch (e) {
      prismaOk = false;
    }
    ...
    try {
      ...
      // Scores
      await (Score as any).create({
        eventId,
        teamId: teamAId,
        submissionId: String(subA._id),
        judgeId: organizerId,
        criteria: [{ name: "Impact", score: 7, max: 10 }],
        total: 7,
      });
      await (Score as any).create({
        eventId,
        teamId: teamAId,
        submissionId: String(subA._id),
        judgeId: organizerId + "_2",
        criteria: [{ name: "Impact", score: 8, max: 10 }],
        total: 8,
      });
      await (Score as any).create({
        eventId,
        ...
        judgeId: organizerId,
        criteria: [{ name: "Impact", score: 6, max: 10 }],
        total: 6,
      });
      ...
      await (PlagiarismReport as any).create({
        ...
        similarities: [
          {
            otherSubmissionId: String(subB._id),
            otherRepoUrl: "https://github.com/demo/team-beta",
            similarity: 0.2,
          },
        ],
        status: "COMPLETED",
